...
The Core product surface is exactly `/setup`, `/ready`, `/go`, and `/doctor`. Preserve existing project entry files during setup. A same-session `/go` approves and activates only the exact contract just presented; critical actions still require separate confirmation. Store only safe evidence metadata.
