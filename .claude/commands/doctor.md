...
| Hook Surface (existence) | `hook_surface_issues` | hook 스크립트 경로 정정 또는 복구 |
| **Hook Parity (registration)** | `hook_parity_issues` | 4 환경(`.claude`/`.codex`/`.gemini`/`.cursor`) hook 등록 동기화 |
| **Hook Matcher Drift** | `hook_matcher_drift_issues` | 같은 hook 이 환경별로 다른 matcher 를 갖는 경우 통일 |
| **Inline Hooks** | `inline_hook_issues` | 인라인 `python -c` / `node -e` 의도 검토 (환경별 비교) |
...
- `recommended_actions` — `{id, action_type, target_path, proposed_change, risk_level, rationale, source_issue}` 구조의 고정 스키마. 별도 도구가 그대로 소비할 수 있다.
